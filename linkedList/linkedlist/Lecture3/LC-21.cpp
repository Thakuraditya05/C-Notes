...
int main(){
// m1 bu spaceComplexity o(m+n), t(m+n); extra node concept 
// temp node -1 value ke saath  and t bhi banega ussi point pr traverse krne ke liye kaam aayega 
//  a->val >=b->val t->next=b;  t=t->next; a=a->next;
// a->val <=b->val t->next=a;   t=t->next; b=b->next;
...
//sc=O(m+n)tc(m+n)
        ...
        //         ListNode* t =new Node(tempA->val);// bode bana di uss value ki 
        //         tempC->next=t;
        //         tempC=t;
        //     }
        ...
        // m2 s=o(1);
        // if(a->val<=b->val) temp->next=a; a=a->next t=t->next
        // a!=null ||b1!=NULL
        // if(a->vaal<b->val) temp->next=b; b=b->next  t=t->next 
        //  a!=null ||b1!=NULL
